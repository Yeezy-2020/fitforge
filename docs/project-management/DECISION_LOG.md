...
| 2026-07-09 | Use cycle-neutral progression naming for training programs; keep legacy `periodWeeks` JSON read compatibility only. | Current program behavior is based on plan cycles/rounds, not natural calendar weeks. | New saves write `periodCycles`; old local data with `periodWeeks` can still be read. | `lib/data/models/training_program.dart`, `test/features/training_program_test.dart` |
